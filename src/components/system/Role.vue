    ...
    <el-dialog title="添加角色信息" :visible.sync="addRoleVisible" width="60%" @close="addRoleClosed">
      ...
      <el-form :model="addForm" ref="addFormRef" label-width="100px">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="角色关键词：" prop="keyword">
              <el-input v-model="addForm.keyword" placeholder="请输入角色关键词"></el-input>
            </el-form-item>
            <div class="grid-content bg-purple-light"></div>
          </el-col>
          <el-col :span="12">
...
export default {
  ...
  methods: {
    //根据分页获取对应的会员列表
    async queryAllRole() {
      const {data: res} = await this.$http.get('/api/role.action', {
    ...
    addRole() {
      this.$refs.addFormRef.validate(async valid => {
        if (!valid) return
        //可以发起添加角色的网络请求
        const {date: res} = await this.$http.post('/api/addrole.action', this.addForm)
    ...
    //展示编辑角色的对话框
    async queryRole(id) {
      const {date: res} = await this.$http.get('/api/queryrole.action/' + id)
      this.editForm = res
    ...
    updateUser() {
      this.$refs.editFormRef.validate(async valid => {
        ...
        const {date: res} = await this.$http.put(
            '/api/updateRole.action', this.editForm)
        //关闭对话框
    ...
    async delRole(id) {
      ...
      const {date: res} = await this.$http.delete('/api/delRole.action/${id}')
